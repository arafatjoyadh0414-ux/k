...
const GuardianBot = () => {
  ...
  return (
    <>
      {/* Floating "Ask Mr Genius" trigger — bottom-left, refined & sophisticated */}
      <button
        type="button"
        data-testid="guardian-bot-trigger"
        onClick={() => setOpen((v) => !v)}
        className={`fixed bottom-4 left-3 sm:bottom-5 sm:left-5 z-40 group inline-flex items-center gap-1.5 sm:gap-2 pl-2 sm:pl-2.5 pr-2.5 sm:pr-3 py-1.5 sm:py-2 rounded-full shadow-lg backdrop-blur-md transition-all hover:-translate-y-0.5 hover:shadow-xl ${
          open
            ? "bg-zinc-900 text-white border border-white/10"
            : "bg-zinc-950/95 text-white border border-[#E11D48]/40 hover:border-[#E11D48]"
        }`}
        aria-label="Ask Mr Genius — JOY's AI automotive assistant"
      >
        <span className={`relative grid place-items-center w-5 h-5 sm:w-[22px] sm:h-[22px] rounded-full ${open ? "bg-white/10" : "bg-[#E11D48]"}`}>
          {open ? <X className="w-3 h-3 sm:w-3.5 sm:h-3.5" /> : <Sparkles className="w-3 h-3 sm:w-3.5 sm:h-3.5" />}
          {!open && <span className="absolute inset-0 rounded-full bg-[#E11D48]/40 animate-ping opacity-60" />}
        </span>
        <span className="font-mono text-[9px] sm:text-[10px] uppercase tracking-[0.22em] sm:tracking-[0.24em] font-semibold whitespace-nowrap">
          {open ? "Close" : "Mr Genius"}
        </span>
      </button>
