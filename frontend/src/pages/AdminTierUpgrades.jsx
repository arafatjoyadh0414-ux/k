        ...
        {loading ? (
          ...
          <div className="industrial-card overflow-hidden" data-testid="tier-upgrades-list">
            ...
            {items.map((u) => (
              <div
                key={u.log_id}
                className="grid grid-cols-12 px-5 py-4 items-center border-b border-slate-100 last:border-b-0 text-sm hover:bg-slate-50 transition-colors"
                data-testid={`tier-upgrade-${u.log_id}`}
              >
                <div className="col-span-3">
                  <div className="font-semibold truncate">{u.company_name || "—"}</div>
                  <div className="text-[10px] font-mono text-slate-500">{u.workshop_id}</div>
                </div>
                ...
                <div className="col-span-3">
                  <div className="flex items-center gap-2">
                    {u.from_tier !== u.to_tier ? (
                      <>
                        <span className={`text-[10px] uppercase tracking-[0.1em] px-2 py-1 border rounded-sm ${TIER_COLOR[u.from_tier] || TIER_COLOR.silver}`}>
                          {u.from_tier}
                        </span>
                        <ArrowRight className="w-3 h-3 text-slate-400" />
                        <span className={`text-[10px] uppercase tracking-[0.1em] px-2 py-1 border rounded-sm font-bold ${TIER_COLOR[u.to_tier] || TIER_COLOR.silver}`}>
                          {u.to_tier}
                        </span>
                      </>
                    ) : (
                      <span className={`text-[10px] uppercase tracking-[0.1em] px-2 py-1 border rounded-sm ${TIER_COLOR[u.to_tier] || TIER_COLOR.silver}`}>
                        {u.to_tier} (credit only)
                      </span>
                    )}
                  </div>
                </div>
